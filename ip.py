...
def target(ip):
    ...
    results = re.findall(
        '<span class="Whwtdhalf w15-0">(.*?)</span>.*?<span class="Whwtdhalf w15-0">(.*?)</span>.*?<span class="Whwtdhalf w15-0">(.*?)</span>.*?<span class="Whwtdhalf w50-0">(.*?)</span>', content, re.S)
