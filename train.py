...
def train(checi):
    ...
    try:
        txt = results[0][2]+'\n'+results[0][0]+u'次'+'\n'+results[0][3] + \
            u'开'+results[0][4]+u'到'+u'全程运行'+results[0][1]+'\n\n'
    except IndexError, e:
        txt = u'没有找到这个车次(⊙o⊙)哦，可能是临客，或者......你输错了吧还是想搞事情？'
        return txt.encode('utf-8')
    txts = re.findall(
        '</td><td align="center"><a href="http://search.huochepiao.com/chezhan/.*?">(.*?)</a></td>.*?<td align="center">(.*?)</td><td align="center">(.*?)</td>', content, re.S)
