...
def Joke():
    ...
    items=re.findall(r'<h2>(.*?)</h2>.*?<div class="content">.*?<span>(.*?)</span>.*?</div>',content,re.S)
...
def Duanzi():
    ...
    data=webPage.read()
    content=data.decode('utf-8')
    items=re.findall('<span class="name">(.*?)</span>.*?<div class=.*?<h1 class="title">.*?<p>(.*?)</p>.*?</h1>',content,re.S)
    m=random.randint(0,15)
    i=m
    while i<m+5:
        jokes=jokes+u'作者:  '+items[i][0]+'\n\n'+items[i][1]+'\n\n\n'
...
def Pic():
    ...
    data=webPage.read()
    content=data.decode('utf-8')
    items=re.findall('<div class="img-wrapper">.*?data-src="(.*?)".*?data-image-info',content,re.S)
    m=random.randint(0,15)
    i=m
    while i<m+6:
        jokes=jokes+items[i]+'\n\n'
...
def WhuPmc():
    ...
    results=re.findall('<div class="h2"><a href="/list/1">.*?</a></div>(.*?)<div class="h2"><a href="/list/2">.*?</a></div>',content,re.S)
    titles=re.findall('<li><a href="(.*?)" target="_self" title=\'(.*?)\'>\r\n\t(.*?)\r\n&nbsp;',results[0],re.S)
    txt=''
    m=len(titles)
    n=0
    while n<m:
...
def BitSmen():
    ...
    titles=re.findall('<a href="(.*?)" title="(.*?)" target="_blank">',results[0],re.S)
